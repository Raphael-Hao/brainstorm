...
def serial_expert(batch, E, M, K, N, out_dtype="float32"):
    ...
    weights = [
        te.placeholder((1, N, K), dtype=out_dtype, name="weight") for i in range(E)
    ]
    outs = [
        topi.nn.matmul(
            data,
...
def main():
    ...
    argparser.add_argument("--task", type=str, default="tune", required= True, choices=["search", "benchmark"])
    argparser.add_argument("--type", type=str, default="all", required=True, choices=["all", "fusion", "serial"])
    ...
    if args.task == "search":
        if args.type == "all":
            search_expert_kernel(args.batch, args.E, args.M, args.K, args.N, expert_kernel=fusion_expert)
            search_expert_kernel(args.batch, args.E, args.M, args.K, args.N, expert_kernel=serial_expert)
        ...
        if args.type == "all":
            benchmark_expert_kernel(args.batch, args.E, args.M, args.K, args.N, expert_kernel=fusion_expert)
            benchmark_expert_kernel(args.batch, args.E, args.M, args.K, args.N, expert_kernel=serial_expert)
        elif args.type == "fusion":
            benchmark_expert_kernel(args.batch, args.E, args.M, args.K, args.N, expert_kernel=fusion_expert)
        elif args.type == "serial":
            benchmark_expert_kernel(args.batch, args.E, args.M, args.K, args.N, expert_kernel=serial_expert)
