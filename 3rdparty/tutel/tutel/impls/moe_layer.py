...
class router_exporter:
    ...
    def add_indices(indices):
        assert len(router_result) > 0
        router_result[-1]['expert'][router_exporter.get_entry()] = [ _.cpu() for _ in indices]
    ...
    def dump():
        global dump_id
        pickle.dump(router_exporter.get_router_result(), open(router_exporter.get_path()+f'.{dump_id}', 'wb'))
        dump_id += 1
...
class PrimFwdAllgather(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        output = torch.empty([num_nodes, input.numel()], device=input.device, dtype=input.dtype)
        tensor_list = [x.contiguous() for x in torch.chunk(output, chunks=num_nodes, dim=0)]
    ...
    def backward(ctx, doutput):
        ...
        dinput = doutput.contiguous().view(world_size, -1).contiguous()[world_rank, :].view(
            ctx.input_shape).contiguous()
        return dinput
...
class TopKGate(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.vitmoe_loss:
            print("[warning] change use_noise in TopKGate to True because vitmoe_loss is set to True")
            self.use_noise = True
    ...
    def apply_on_expert_fn(self, input, expert_fn, group, sharded_count):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            if self.num_global_experts <= 1:
                ...
                l_loss = vitmoe_loss(gates_wo_noise, topk_logits, self.num_global_experts, self.use_global_loss)
            else:
        ...
        if self.top_k > 1:
            ...
            for k in range(1, self.top_k):
                acc_base = torch.sum(masks_se[k - 1], dim=0,
                                     keepdim=True) if acc_base is None else acc_base + torch.sum(masks_se[k - 1], dim=0,
                                                                                                 keepdim=True)
        ...
        if not hasattr(self, '_fdr'):
            capacity = self.top_k * int(
                self.capacity_factor * ((S + self.num_global_experts - 1) // self.num_global_experts))
            self._fdr = fast_dispatcher(num_global_experts=GE, capacity=capacity, model_dim=M,
...
class MOELayer(torch.nn.Module):
    ...
    def __init__(self, gate_type, model_dim: int, experts=None, scan_expert_func=None, result_func=None,
                 group: Optional[Any] = None, seeds=None,
                 normalize_gate=True, batch_prioritized_routing=False, vitmoe_loss=False, use_global_loss=False,
                 use_noise=True, capacity_factor=1.0, mlpfp32=False, has_fc2_bias=True, is_postscore=True,
                 a2a_ffn_overlap_degree=1, **kwargs):
        super().__init__()
        ...
        if self.num_local_experts < 0:
            ...
            assert experts[
                       'hidden_size_per_expert'] % sharded_count == 0, f"Cannot evenly divide hidden_size_per_expert ({experts['hidden_size_per_expert']}) to {sharded_count} slices."
            self.num_global_experts = num_devices // sharded_count
            self.num_local_experts, experts['hidden_size_per_expert'] = 1, experts[
                'hidden_size_per_expert'] // sharded_count
        ...
        if not isinstance(experts, dict):
            ...
            if experts['type'] == 'ffn':
                ...
                class FusedExpertsNetwork(torch.nn.Module):
                    def __init__(self, model_dim, hidden_size, local_experts, ffn_zero_group, mlpfp32=False,
                                 has_fc2_bias=True):
                        super().__init__()
                        ...
                        fc2_bias = torch.empty(1, local_experts, 1, (model_dim + sharded_count - 1) // sharded_count) \
                            if self.has_fc2_bias else None

                        for i in range(local_experts):
                            fc1 = torch.nn.Linear(model_dim, hidden_size)
                            fc2 = torch.nn.Linear(hidden_size, model_dim, bias=self.has_fc2_bias)
                    ...
                    def extra_repr(self):
                        return 'model_dim=%d, hidden_size=%d, local_experts=%d, bias=%s' % (
                            self.model_dim, self.hidden_size, self.local_experts, self.fc1_bias is not None)
                    ...
                    def forward(self, x):
                        ...
                        fc1_weight, fc2_weight, fc1_bias = self.fc1_weight, self.fc2_weight, self.fc1_bias
                        if self.has_fc2_bias:
                        ...
                        if self.local_experts == 1:
                            ...
                            with torch.cuda.amp.autocast(enabled=False):
                                x = torch.addmm(fc1_bias.unsqueeze(0).float(), x.float(), fc1_weight.t().float())
                            x = activation_fn(x.unsqueeze(0)).squeeze(0)
                            x = self.dropout_fc1(x)
                            if self.mlpfp32:
                                with torch.cuda.amp.autocast(enabled=False):
                                    if self.has_fc2_bias:
                                        x = torch.addmm(fc2_bias.unsqueeze(0).float(), x.float(), fc2_weight.float())
                                    else:
                            ...
                            with torch.cuda.amp.autocast(enabled=False):
                                x = torch.matmul(x.float(), fc1_weight.swapaxes(1, 2).float()) + fc1_bias.float()
                            x = activation_fn(x)
                            x = self.dropout_fc1(x)
                ...
                self.experts = ModuleList([FusedExpertsNetwork(model_dim, experts['hidden_size_per_expert'],
                                                               self.num_local_experts, self.ffn_zero_group,
                                                               mlpfp32=self.mlpfp32, has_fc2_bias=self.has_fc2_bias)])
            else:
        ...
        for gi, single_gate_type in enumerate(gate_type):
            if single_gate_type['type'] == 'top':
                ...
                self.gates += [TopKGate(model_dim=model_dim, top_k=single_gate_type['k'],
    ...
    def forward(self, input: Tensor, gate_index=0, **kwargs: Any):
            ...
            if reshaped_input.size(0) > self.expected_sample_size:
                raise Exception(
                    'MoE JIT is designed to work on sample size = %s, while receiving sample size = %s (> %s)' % (
                        self.expected_sample_size, reshaped_input.size(0), self.expected_sample_size))
            else:
                ...
                pad_input = torch.zeros([self.expected_sample_size, self.model_dim], dtype=reshaped_input.dtype,
                                        layout=reshaped_input.layout, device=reshaped_input.device)
        ...
        result_output, l_aux = self.gates[gate_index].apply_on_expert_fn(reshaped_input, self.expert_fn, self.group,
                                                                         sharded_count=self.sharded_count)
