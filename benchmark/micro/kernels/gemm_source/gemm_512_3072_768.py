...
source_code = r"""
extern "C" __global__ void __launch_bounds__(256) sgemm_512x3072x768(float* __restrict__ placeholder, float* __restrict__ placeholder1, float* __restrict__ T_matmul_NT) {
  ...
  // [thread_extent] threadIdx.z = 1
    ...
    const char *A_ldg_ptr = (const char *)(
        placeholder + (blockIdx.y * 128 + threadIdx.x / 8 * 4) * 3072 + threadIdx.x % 8);

    // const char *B_ldg_ptr = (const char *)(
    //     placeholder1 + (threadIdx.x / 32) * 768 + blockIdx.x * 128 + threadIdx.x % 32);
    const char *B_ldg_ptr = (const char *)(
        placeholder1 + (blockIdx.x * 128 + threadIdx.x / 8 * 4) * 3072 + threadIdx.x % 8);

    // A_tile & B_tile sts/lds pointer
    // using uint pointer for faster double buffer switch
    uint A_sts_addr = smem_u32addr(
        A_smem + (threadIdx.x % 8) * 132 + (threadIdx.x / 8) * 4);
    // uint B_sts_addr = smem_u32addr(
    //     B_smem + (threadIdx.x / 32) * 128 + (threadIdx.x % 32));
    uint B_sts_addr = smem_u32addr(
        B_smem + (threadIdx.x % 8) * 128 + (threadIdx.x / 8) * 4);;

    uint A_lds_addr = smem_u32addr(
        A_smem + (warp_id / 2) * 32 + mma_tid_y * 4);
    uint B_lds_addr = smem_u32addr(
    ...
    uint k_tiles = (3072 + 7) / 8 - 1;

    // load 1'st tile to shared memory
    {
        uint first_k_tile = 3072 - k_tiles * 8;
    ...
    for (; k_tiles > 0; --k_tiles) {
        ...
        for (int k_frag = 0; k_frag < 8; ++k_frag) {
            ...
            // load next placeholder&placeholder1 fragment from shared memory to register
    ...
    // C_tile write back, reuse placeholder&placeholder1 tile shared memory buffer
    ...
    float *C_stg_ptr = T_matmul_NT + m_idx * 768 + n_idx;

    if (m_idx >= 512) {
        return;
    } else if (m_idx + 32 <= 512) {
        uint n_guard = 768 < n_idx ? 0 : 768 - n_idx;
