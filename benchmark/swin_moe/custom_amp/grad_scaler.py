    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def update(self, new_scale=None):
        ...
        if new_scale is not None:
            ...
            _cur_inf = found_inf_combined.item() > 0

            # print(f"[{_scale.device}]====> _scale:{_scale} _growth_tracker:{_growth_tracker} "
            #       f"found_inf_combined{found_inf_combined} _last_inf:{self._last_inf} "
            #       f"_cur_inf={_cur_inf}")
            #
            if not self._last_inf:
                found_inf_combined *= 0
