...
from utils import load_checkpoint, save_checkpoint, auto_resume_helper, reduce_tensor, \
...
def parse_option():
    ...
    parser.add_argument('--zero_opt', type=int, default=0, help='zero_optimization level')
    parser.add_argument('--dpfp16', action='store_true', default=False, help='deepspeed fp16')
...
def main(args, config, ds_init):
    ...
    for name, param in model.named_parameters():
        if hasattr(param, 'skip_allreduce') and param.skip_allreduce is True:
            ...
            print(f"[{name}] skip_allreduce and div {config.TRAIN.MOE_GRAD_SCALE} for grad")
    ...
    if config.ENABLE_DEEPSPEED:
        ...
        model, optimizer, _, _ = ds_init(
            args=args, model=model, model_parameters=parameters, dist_init_required=False,
        )
        logger.info("========> DeepSpeed initializd!!!!!!")
        logger.info(f"model.gradient_accumulation_steps() = {model.gradient_accumulation_steps()}")
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(config, model, model_without_ddp, criterion, data_loader_train, optimizer, epoch, mixup_fn,
        ...
        logger.info(f"rank[{dist.get_rank()}] Save checkpoint takes {time.time() - tic}s")
...
def train_one_epoch(config, model, model_without_ddp, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler,
                    loss_scaler):
    model.train()
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if loss_scaler is None:
            ...
            grad_norm = -1
            if config.DPFP16:
                loss_scale_value = model.optimizer.loss_scale if hasattr(model.optimizer, "loss_scale") else \
                    model.optimizer.cur_scale
            else:
                loss_scale_value = -1
        else:
        ...
        loss_meter.update(loss.item() * config.TRAIN.ACCUMULATION_STEPS, targets.size(0))  # todo: modify in 2022/02/24
        ...
        if config.TRAIN.SAVE_STEP > 0 and (idx + 1) % config.TRAIN.SAVE_STEP == 0:
            if config.TRAIN.ACCUMULATION_STEPS > 1 and (idx + 1) % config.TRAIN.ACCUMULATION_STEPS != 0:
                ...
                tic = time.time()
                save_checkpoint(config, epoch, model_without_ddp, -1.0, optimizer, lr_scheduler, logger,
                                step=idx, loss_scaler=loss_scaler)
                print(f"rank[{dist.get_rank()}] Save checkpoint takes {time.time() - tic}s")
                logger.info(f"rank[{dist.get_rank()}] Save checkpoint takes {time.time() - tic}s")
                batch_time.reset()
                data_time.reset()
...
if __name__ == '__main__':
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        print(f"RANK and WORLD_SIZE in environ: {rank}/{world_size}, LOCAL_RANK: {config.LOCAL_RANK}, "
    ...
    if config.TRAIN.ACCUMULATION_STEPS > 1:
        ...
        if config.TRAIN.SAVE_STEP > 0:
            config.TRAIN.SAVE_STEP = config.TRAIN.SAVE_STEP * config.TRAIN.ACCUMULATION_STEPS
    config.TRAIN.BASE_LR = config.TRAIN.TOTAL_LR if config.TRAIN.TOTAL_LR > 0 else linear_scaled_lr
    config.TRAIN.WARMUP_LR = config.TRAIN.TOTAL_WARMUP_LR if config.TRAIN.TOTAL_WARMUP_LR > 0 else linear_scaled_warmup_lr
    config.TRAIN.MIN_LR = config.TRAIN.TOTAL_MIN_LR if config.TRAIN.TOTAL_MIN_LR > 0 else linear_scaled_min_lr
    config.freeze()
