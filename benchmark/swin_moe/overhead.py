...
from brt.runtime.benchmark import deterministic_random_generator, profile_v2, CUDATimer
...
def main(args, config, ds_init):
    ...
    if args.mode == "gather-ckpt":
        ...
        overhead(config, data_loader_val, model)
...
@torch.no_grad()
def overhead(config, data_loader, model):
    model.eval()
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        cuda_timer = CUDATimer(loop=50, repeat=5)
        cuda_timer.execute(lambda: model(images), msg="capture mode")
        model = switch_router_mode(model, False).eval()
        cuda_timer.execute(lambda: model(images), msg="normal mode")
        break
