...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
                 mlpfp32=False, mlp_fc2_bias=True):
        super().__init__()
...
class MoEMlp(nn.Module):
    def __init__(self, model_dim, hidden_size, num_local_experts, top_value, capacity_factor=1.25,
                 normalize_gate=True, batch_prioritized_routing=False,
                 vitmoe_loss=False, use_global_loss=False, use_noise=True,
                 moe_drop=0.0, init_std=0.02, mlpfp32=False, mlp_fc2_bias=True,
                 moe_post_score=True, temperature_init=0.5, norm_layer=None):
        super().__init__()
...
class ConvMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
                 mlpfp32=False, proj_ln=False, mlp_fc2_bias=True):
        super().__init__()
        self.mlp = Mlp(in_features=in_features, hidden_features=hidden_features, out_features=out_features,
                       act_layer=act_layer, drop=drop, mlpfp32=mlpfp32, mlp_fc2_bias=mlp_fc2_bias)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 ...
                 rpe_output_type='normal', attn_type='normal', mlpfp32=False, rm_k_bias=True):
...
class SwinTransformerBlockPost(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 endnorm=False, act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 relative_coords_table_type='norm8_log', rpe_hidden_dim=512,
                 rpe_output_type='normal', attn_type='normal', mlp_type='normal', mlpfp32=False,
                 is_moe=False, num_local_experts=1, top_value=2, capacity_factor=1.25,
                 normalize_gate=True, batch_prioritized_routing=False,
                 vitmoe_loss=False, use_global_loss=False, use_noise=True, mlp_fc2_bias=True,
                 moe_drop=0.0, moe_post_score=True, moe_temperature_init=0.5,
                 init_std=0.02, rm_k_bias=True, skip_moe_droppath=False, norm_in_moe=False):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            rpe_hidden_dim=rpe_hidden_dim, attn_type=attn_type, mlpfp32=mlpfp32, rm_k_bias=rm_k_bias)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = nn.Identity() if (self.is_moe and self.norm_in_moe) else norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        if self.is_moe:
            self.mlp = MoEMlp(model_dim=dim, hidden_size=mlp_hidden_dim,
                              num_local_experts=num_local_experts, top_value=top_value,
                              capacity_factor=capacity_factor, normalize_gate=normalize_gate,
                              batch_prioritized_routing=batch_prioritized_routing,
                              vitmoe_loss=vitmoe_loss, use_global_loss=use_global_loss, use_noise=use_noise,
                              moe_drop=moe_drop, init_std=init_std, mlpfp32=mlpfp32, mlp_fc2_bias=mlp_fc2_bias,
                              moe_post_score=moe_post_score, temperature_init=moe_temperature_init,
                              norm_layer=norm_layer if norm_in_moe else None)
        else:
            if mlp_type == 'normal':
                self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                               mlpfp32=mlpfp32, mlp_fc2_bias=mlp_fc2_bias)
            elif mlp_type == 'conv':
                self.mlp = ConvMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                                   mlpfp32=mlpfp32, mlp_fc2_bias=mlp_fc2_bias)
            elif mlp_type == 'conv_ln':
                self.mlp = ConvMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                                   mlpfp32=mlpfp32, proj_ln=True, mlp_fc2_bias=mlp_fc2_bias)
...
class SwinTransformerBlockPre(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 endnorm=False, act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 init_values=-1.0, relative_coords_table_type='norm8_log', rpe_hidden_dim=512,
                 rpe_output_type='normal', attn_type='normal', mlp_type='normal', mlpfp32=False,
                 is_moe=False, num_local_experts=1, top_value=2, capacity_factor=1.25,
                 normalize_gate=True, batch_prioritized_routing=False,
                 vitmoe_loss=False, use_global_loss=False, use_noise=True, mlp_fc2_bias=True,
                 moe_drop=0.0, moe_post_score=True, moe_temperature_init=0.5,
                 init_std=0.02, rm_k_bias=True, skip_moe_droppath=False):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 checkpoint_blocks=255, init_values=-1.0, endnorm_interval=-1,
                 relative_coords_table_type='norm8_log', rpe_hidden_dim=512,
                 rpe_output_type='normal', attn_type='normal', mlp_type='normal',
                 mlpfp32_blocks=[-1], postnorm=True,
                 moe_block=[-1], num_local_experts=1, top_value=2, capacity_factor=1.25,
                 normalize_gate=True, batch_prioritized_routing=False,
                 vitmoe_loss=False, use_global_loss=False, use_noise=True, mlp_fc2_bias=True,
                 moe_drop=0.0, moe_post_score=True, moe_temperature_init=0.5,
                 init_std=0.02, aux_loss_scale=[1.0], rm_k_bias=True, skip_moe_droppath=False, norm_in_moe=False):
        ...
        if self.postnorm:
            ...
            self.blocks = nn.ModuleList([
                SwinTransformerBlockPre(dim=dim, input_resolution=input_resolution,
                                        num_heads=num_heads, window_size=window_size,
                                        shift_size=0 if (i % 2 == 0) else window_size // 2,
                                        mlp_ratio=mlp_ratio,
                                        qkv_bias=qkv_bias, qk_scale=qk_scale,
                                        ...
                                        drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                                        norm_layer=norm_layer,
                                        init_values=init_values[i] if isinstance(init_values, list) else init_values,
                                        endnorm=True if ((i + 1) % endnorm_interval == 0) and (
                                                endnorm_interval > 0) else False,
                                        relative_coords_table_type=relative_coords_table_type,
                                        rpe_hidden_dim=rpe_hidden_dim,
        ...
        if downsample is not None:
            self.downsample = downsample(input_resolution, dim=dim, norm_layer=norm_layer, postnorm=postnorm)
        else:
    ...
    def forward(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            if self.use_checkpoint and not blk.is_moe and ckpt_block < self.checkpoint_blocks:
...
class SwinV2TransformerMoE(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, moe_blocks=[[-1], [-1], [-1], [-1]],
                 init_values=[1e-5], endnorm_interval=-1,
                 relative_coords_table_type='norm8_log', rpe_hidden_dim=512,
                 attn_type='cosine_mh', rpe_output_type='sigmoid',
                 checkpoint_blocks=[255, 255, 255, 255],
                 mlpfp32_layer_blocks=[[-1], [-1], [-1], [-1]],
                 postnorm=True, mlp_type='normal', patch_embed_type='normal', patch_merge_type='normal',
                 droppath_rule='linear', strid16=False, strid16_global=False,
                 num_local_experts=1, top_value=2, capacity_factor=1.25,
                 normalize_gate=True, batch_prioritized_routing=False,
                 vitmoe_loss=False, use_global_loss=False, use_noise=True, mlp_fc2_bias=True,
                 moe_drop=0.0, moe_post_score=True, moe_temperature_init=0.5, init_std=0.02,
                 aux_loss_scales=[[1.0], [1.0], [1.0], [1.0]], rm_k_bias=True,
                 skip_moe_droppath=False, head_2fc=False, norm_in_moe=False, **kwargs):
        super().__init__()
        ...
        self.num_features = int(embed_dim * 2 ** (self.num_layers - 2)) if strid16 \
        ...
        if patch_embed_type == 'normal':
            ...
            self.patch_embed = ResNetDLNPatchEmbed(
                img_size=img_size, in_chans=in_chans, embed_dim=embed_dim, norm_layer=norm_layer)
        elif patch_embed_type == 'resnetdnf':
            assert patch_size == 4, "check"
            self.patch_embed = ResNetDLNPatchEmbed(
                img_size=img_size, in_chans=in_chans, embed_dim=embed_dim, norm_layer=None)
        ...
        if len(init_values) == 1:
            ...
            initv = [x.item() for x in torch.linspace(init_values[0], init_values[1], sum(depths))]
        else:
            raise NotImplementedError
        ...
        for i_layer in range(self.num_layers):
            ...
            cur_input_resolution = (patches_resolution[0] // (2 ** (i_layer - 1)),
