...
class MoEMlp(nn.Module):
    def __init__(self, model_dim, hidden_size, num_local_experts, top_value, capacity_factor=1.25,
                 ...
                 vitmoe_loss=False, use_global_loss=False, mlp_fc2_bias=True,
                 moe_drop=0.0, init_std=0.02):
        super().__init__()
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.is_moe:
            self.mlp = MoEMlp(model_dim=dim, hidden_size=mlp_hidden_dim,
                              ...
                              batch_prioritized_routing=batch_prioritized_routing,
                              vitmoe_loss=vitmoe_loss, use_global_loss=use_global_loss, mlp_fc2_bias=mlp_fc2_bias,
                              moe_drop=moe_drop, init_std=init_std)
        else:
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 normalize_gate=True, batch_prioritized_routing=False,
                 vitmoe_loss=False, use_global_loss=False, mlp_fc2_bias=True,
                 moe_drop=0.0, init_std=0.02, aux_loss_scale=[1.0]):
    ...
    def forward(self, x):
        l_aux = 0.0
        for id_blk, blk in enumerate(self.blocks):
            if self.use_checkpoint and not blk.is_moe:
                x, cur_l_aux = checkpoint.checkpoint(blk, x)
            else:
                x, cur_l_aux = blk(x)
            l_aux = cur_l_aux * self.aux_loss_scale[id_blk] + l_aux
...
class SwinTransformerMoE(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 normalize_gate=True, batch_prioritized_routing=False,
                 vitmoe_loss=False, use_global_loss=False, mlp_fc2_bias=True,
                 moe_drop=0.0, init_std=0.02, aux_loss_scales=[[1.0], [1.0], [1.0], [1.0]], **kwargs):
        super().__init__()
