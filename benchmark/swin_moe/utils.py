...
def gather_all_ckpts_into_one(
    ...
    for i in range(num_rank):
        checkpoint.append(
            torch.load(config.MODEL.RESUME + f".rank{i}", map_location="cpu")
        )
    ...
    if loss_scaler is not None:
        if config.SINGLE_GPU_EVAL:
            if config.MODEL.RESUME.endswith(f".rank{global_rank}"):
                ...
                for i in range(num_rank):
                    checkpoint.append(
                        torch.load(
                            config.MODEL.RESUME + f".rank{i}", map_location="cpu"
                        )
                    )
                if local_expert_ckpt < 0:
                    expert_span = -local_expert_ckpt
                        ...
                        if "_bias" in entry:
                            state_dict[entry] = torch.stack(
                                [
                                    ...
                                    for x in new_entry_weights[entry]
                                ]
                            )
                        else:
                            state_dict[entry] = torch.stack(new_entry_weights[entry])
                    ...
                    raise NotImplementedError(
                        f"local_expert_ckpt > 0 has not implemented yet"
                    )
            ...
            if config.MODEL.RESUME.endswith(f".rank{global_rank}"):
                ...
                config.freeze()
                logger.info(
                    f"===> Rank[{global_rank}] Re-formatting checkpoint to {config.MODEL.RESUME}......"
                )
                print(
                    f"===> Rank[{global_rank}] Re-formatting checkpoint to {config.MODEL.RESUME}......"
                )
        ...
        if (
            ...
            if "scaler" in checkpoint:
                loss_scaler.load_state_dict(checkpoint["scaler"])
            logger.info(
                f"=> Rank[{global_rank}] loaded successfully "
                f"'{config.MODEL.RESUME}' (epoch {checkpoint['epoch']})"
            )
            print(
                f"=> Rank[{global_rank}] loaded successfully "
...
def load_pretrained(config, model, logger):
    global_rank = dist.get_rank()
    logger.info(
        f"==============> Rank[{global_rank}] Loading weight {config.MODEL.PRETRAINED} for finetuning......"
    )
    print(
        f"==============> Rank[{global_rank}] Loading weight {config.MODEL.PRETRAINED} for finetuning......"
    )
    ...
    if config.MODEL.PRETRAINED.endswith(f".rank{global_rank}"):
        ...
        config.freeze()
        logger.info(
            f"===> Rank[{global_rank}] Re-formatting checkpoint to {config.MODEL.PRETRAINED}......"
        )
        print(
            f"===> Rank[{global_rank}] Re-formatting checkpoint to {config.MODEL.PRETRAINED}......"
        )
    ...
    moe_fc1_weight_keys = [
        k for k in state_dict.keys() if "_moe_layer.experts.0.fc1_weight" in k
    ]
    for k in moe_fc1_weight_keys:
        moe_fc1_weight_pretrained = state_dict[k]
        moe_fc1_weight_current = model.state_dict()[k]
        if (
            len(moe_fc1_weight_pretrained.shape) == 2
            and len(moe_fc1_weight_current.shape) == 2
        ):
    ...
    for k in moe_fc1_bias_keys:
        ...
        if (
            len(moe_fc1_bias_pretrained.shape) == 2
            and len(moe_fc1_bias_current.shape) == 1
        ):
            assert moe_fc1_bias_pretrained.shape[0] == 1
            state_dict[k] = moe_fc1_bias_pretrained.view(-1)
            logger.info(f"reformatting {k}")
        elif (
            len(moe_fc1_bias_pretrained.shape) == 1
            and len(moe_fc1_bias_current.shape) == 2
        ):
                    ...
                    for i in range(nH1):
                        ...
                        all_rel_pos_bias.append(
                            torch.nn.functional.pad(
                                z, padding, "constant", z.min().item() - 3
                            ).view(L2, 1)
                        )
    ...
    if Nc1 != Nc2:
        if Nc1 == 21841 and Nc2 == 1000:
            ...
            nn.init.constant_(model.head.weight, 0.0)
    ...
    if loss_scaler is not None:
        ...
        if step is not None:
            save_path = os.path.join(
                config.OUTPUT, f"ckpt_epoch_{epoch}_{step}.pth.rank{global_rank}"
            )
        else:
            save_path = os.path.join(
...
def auto_resume_helper(output_dir, logger, deepspeed=False):
    if not deepspeed:
        ...
        checkpoints = [
            ckpt for ckpt in checkpoints if ckpt.endswith(f".pth.rank{global_rank}")
        ]
        logger.info(
            f"All checkpoints for rank {global_rank} founded in {output_dir}: {checkpoints}"
        )
        print(
            f"All checkpoints for rank {global_rank} founded in {output_dir}: {checkpoints}"
        )
        if len(checkpoints) > 0:
            ...
            logger.info(
                f"The latest checkpoint founded for rank {global_rank} : {latest_checkpoint}"
            )
            print(
                f"The latest checkpoint founded for rank {global_rank} : {latest_checkpoint}"
            )
...
def create_ds_config(args, config):
    args.deepspeed_config = os.path.join(
        config.OUTPUT, f"deepspeed_config_{dist.get_rank()}.json"
    )
