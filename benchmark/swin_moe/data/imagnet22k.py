...
class IN21KDATASET(data.Dataset):
    def __init__(self, root, ann_file='', transform=None, target_transform=None):
        super(IN21KDATASET, self).__init__()

        self.data_path = root
        # self.tarreader = TarReader()
        self.tarreader = ZipReader()
        ...
        # some result: https://github.com/dmlc/mxnet-model-gallery/blob/master/imagenet-21k-inception.md
        self.database = json.load(open(self.ann_path))

    def _load_image(self, path):
        if '@' in path:
            # TODO: check tar
            return self.tarreader.imread(path)
        else:
            try:
                im = Image.open(path)
            except:
