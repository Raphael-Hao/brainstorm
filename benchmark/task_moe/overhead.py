...
def main():
    ...
    overhead(config, model_ddp, model, input_ids)
...
def overhead(
    ...
    with torch.inference_mode():
        for i in range(bench_iteration):
            timer = CUDATimer(loop=50, repeat=5)
            timer.execute(
                lambda: model_ddp(input_ids, task_ids=all_task_ids[i]), "capature mode"
            )
            model_ddp = switch_router_mode(model_ddp, False)
            timer.execute(
                lambda: model_ddp(input_ids, task_ids=all_task_ids[i]), "normal mode"
            )
            break
