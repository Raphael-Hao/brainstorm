...
from beam_search import NoWaitBeamSearchScorer
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            # print("Using eos-based greedy search")
        ...
        elif is_no_wait_greedy_gen_mode:
            if num_return_sequences > 1:
                raise ValueError(
                    f"num_return_sequences has to be 1, but is {num_return_sequences} when doing greedy search."
                )
            # print("Using no_wait greedy search")

            # 10. run greedy search
            return self.no_wait_greedy_search(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=pad_token_id,
            ...
            # print("Using eos-based beam search")
        ...
        elif is_no_wait_beam_gen_mode:
            # print("Using no_wait beam search")
            if num_return_sequences > num_beams:
                raise ValueError(
                    "`num_return_sequences` has to be smaller or equal to `num_beams`."
                )
            ...
            # 10. prepare beam search scorer
            beam_scorer = NoWaitBeamSearchScorer(
                batch_size=batch_size,
                num_beams=num_beams,
                device=self.device,
                length_penalty=length_penalty,
