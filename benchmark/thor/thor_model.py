...
class ThorEmbeddings(nn.Module):
...
class ThorSelfAttention(nn.Module):
...
class ThorSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ThorAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = ThorSelfAttention(
            config, position_embedding_type=position_embedding_type
        )
        self.output = ThorSelfOutput(config)
...
class ThorIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.seq_len_dim = 1
        self.attention = ThorAttention(config)
        self.is_decoder = config.is_decoder
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = ThorAttention(
                config, position_embedding_type="absolute"
            )
        if config.expert_num == 1:
    ...
    def feed_forward_chunk(self, attention_output):
        layer_output = self.inter_output(attention_output)
        return layer_output


class ThorEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class ThorPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class ThorLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = ThorPredictionHeadTransform(config)
...
class ThorOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = ThorLMPredictionHead(config)
...
class ThorOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ThorPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = ThorLMPredictionHead(config)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
...
class ThorPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = ThorConfig
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, ThorEncoder):
            module.gradient_checkpointing = value


@dataclass
class ThorForPreTrainingOutput(ModelOutput):
    """
...
class ThorModel(ThorPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = ThorEmbeddings(config)
        self.encoder = ThorEncoder(config)

        self.pooler = ThorPooler(config) if add_pooling_layer else None
...
class ThorForPreTraining(ThorPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.bert = ThorModel(config)
        self.cls = ThorPreTrainingHeads(config)
    ...
    @replace_return_docstrings(
        output_type=ThorForPreTrainingOutput, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
        input_ids=None,
        ...
        return ThorForPreTrainingOutput(
            loss=total_loss,
...
class ThorLMHeadModel(ThorPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = ThorModel(config, add_pooling_layer=False)
        self.cls = ThorOnlyMLMHead(config)
...
class ThorForMaskedLM(ThorPreTrainedModel):
...
class ThorForNextSentencePrediction(ThorPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.bert = ThorModel(config)
        self.cls = ThorOnlyNSPHead(config)
...
class ThorForSequenceClassification(ThorPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class ThorForMultipleChoice(ThorPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class ThorForTokenClassification(ThorPreTrainedModel):
...
class ThorForQuestionAnswering(ThorPreTrainedModel):
