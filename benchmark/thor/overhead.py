...
def main():
    ...
    cuda_timer.execute(
        lambda: thor_moe(x),
        msg=f"brt_capature,{args.expert},{args.token}",
    )
    thor_moe = switch_router_mode(thor_moe, False).eval()
    cuda_timer.execute(
        lambda: thor_moe(x),
        msg=f"brt_nocapture,{args.expert},{args.token}"
    )
