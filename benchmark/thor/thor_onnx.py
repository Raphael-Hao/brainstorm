...
def export_thor_model(model_filename):
    ...
    thor_config.expert_type = "fusion"
    model_filename = (
    ...
    thor_encoder.cuda()
    thor_encoder.eval()
    dummy_input = torch.rand(1, 64, 512, device="cuda", dtype=torch.float32)
    dummy_output = thor_encoder(dummy_input)
    torch.onnx.export(
        thor_encoder, dummy_input, model_filename, verbose=True, opset_version=11
    )
