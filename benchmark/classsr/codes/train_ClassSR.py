        ...
        for _, train_data in enumerate(train_loader):
            ...
            if (
                ...
                if (
                    ...
                    logger.info(
                        "# Validation # TYPE num: {0} {1} {2} ".format(
                            num_ress[0], num_ress[1], num_ress[2]
                        )
                    )
                    logger.info(
                        "# Validation # PSNR Class: {0} {1} {2}".format(
                            psnr_ress[0] / num_ress[0],
                            psnr_ress[1] / num_ress[1],
                            psnr_ress[2] / num_ress[2],
                        )
                    )
                    # tensorboard logger
                    if opt["use_tb_logger"] and "debug" not in opt["name"]:
                        ...
                        tb_logger.add_scalar(
                            "Class1_PSNR", psnr_ress[0] / num_ress[0], current_step
                        )
                        tb_logger.add_scalar(
                            "Class2_PSNR", psnr_ress[1] / num_ress[1], current_step
                        )
                        tb_logger.add_scalar(
                            "class3_PSNR", psnr_ress[2] / num_ress[2], current_step
                        )
