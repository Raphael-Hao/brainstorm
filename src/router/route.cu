...
template <typename dtype, bool weighted, bool max_path_padding, bool tag_generating>
__global__ void __launch_bounds__(1024)
    dispatch_with_tag_indices_1d(dtype* __restrict__ in_data /*[cell_num x cell_size]*/,
                                 dtype* __restrict__ out_data /*[total_load x cell_size]*/,
                                 dtype* __restrict__ gates /*[cell_num x path_num]*/,
                                 int* __restrict__ route_indices /*[cell_num x path_num]*/,
                                 int* __restrict__ loads /*[path_num]*/,
  ...
  for (int i = blockIdx.x; i < cell_num; i += gridDim.x) {
    if (i < path_load) {
      int route_index = i * path_num + blockIdx.y;
      int tag_index = route_indices[route_index];

      if (tag_index == 0) {
        break;
      }
      int real_tag_index = tag_index - 1;
      int global_seat_index = load_start + i;
      int gates_index;
      if (weighted) {
        gates_index = real_tag_index * path_num + blockIdx.y;
      }

      for (int j = threadIdx.x; j < cell_size; j += 1024) {
        if (weighted) {
          out_data[global_seat_index * cell_size + j] =
              in_data[real_tag_index * cell_size + j] * gates[gates_index];
        } else {
          out_data[global_seat_index * cell_size + j] = in_data[real_tag_index * cell_size + j];
        }
      }
      if (threadIdx.x == 0 && tag_generating) {
        new_tags[global_seat_index] = old_tags[real_tag_index];
      }
    }
  }
  if (threadIdx.x == 0 && max_path_padding) {
...
template <typename dtype, bool max_path_padding, bool tag_generating>
__global__ void __launch_bounds__(1024)
    dispatch_with_tag_indices_2d(dtype* __restrict__ in_data /*[path_num x cell_num x cell_size]*/,
                                 dtype* __restrict__ out_data /*[?load*path_num x cell_size]*/,
                                 int* __restrict__ route_indices /*[cell_num x path_num]*/,
                                 int* __restrict__ loads /*[path_num]*/,
                                 int* __restrict__ old_tags,
  ...
  if (!is_tag_index) {
    ...
    if (is_1d_routing) {
      if (gates != nullptr) {
        if (max_path_load != 0) {
          if (old_tags != nullptr) {
            dispatch_with_tag_indices_1d<dtype, true, true, true>
                <<<grid_size, block_size, 0, stream>>>(
                    src_data_ptr, dst_data_ptr, gates_ptr, route_indices, loads, old_tags, new_tags,
                    cell_num, cell_size, path_num, max_path_load);
          } else {
            dispatch_with_tag_indices_1d<dtype, true, true, false>
                <<<grid_size, block_size, 0, stream>>>(
                    src_data_ptr, dst_data_ptr, gates_ptr, route_indices, loads, old_tags, new_tags,
                    cell_num, cell_size, path_num, max_path_load);
          }
          ...
          if (old_tags != nullptr) {
            dispatch_with_tag_indices_1d<dtype, true, false, true>
                <<<grid_size, block_size, 0, stream>>>(
                    src_data_ptr, dst_data_ptr, gates_ptr, route_indices, loads, old_tags, new_tags,
                    cell_num, cell_size, path_num, max_path_load);
          } else {
            dispatch_with_tag_indices_1d<dtype, true, false, false>
                <<<grid_size, block_size, 0, stream>>>(
                    src_data_ptr, dst_data_ptr, gates_ptr, route_indices, loads, old_tags, new_tags,
                    cell_num, cell_size, path_num, max_path_load);
          }
        ...
        if (max_path_load != 0) {
          if (old_tags != nullptr) {
            dispatch_with_tag_indices_1d<dtype, false, true, true>
                <<<grid_size, block_size, 0, stream>>>(
                    src_data_ptr, dst_data_ptr, gates_ptr, route_indices, loads, old_tags, new_tags,
                    cell_num, cell_size, path_num, max_path_load);
          } else {
            dispatch_with_tag_indices_1d<dtype, false, true, false>
                <<<grid_size, block_size, 0, stream>>>(
                    src_data_ptr, dst_data_ptr, gates_ptr, route_indices, loads, old_tags, new_tags,
                    cell_num, cell_size, path_num, max_path_load);
          }
          ...
          if (old_tags != nullptr) {
            dispatch_with_tag_indices_1d<dtype, false, false, true>
                <<<grid_size, block_size, 0, stream>>>(
                    src_data_ptr, dst_data_ptr, gates_ptr, route_indices, loads, old_tags, new_tags,
                    cell_num, cell_size, path_num, max_path_load);
          } else {
            dispatch_with_tag_indices_1d<dtype, false, false, false>
                <<<grid_size, block_size, 0, stream>>>(
                    src_data_ptr, dst_data_ptr, gates_ptr, route_indices, loads, old_tags, new_tags,
                    cell_num, cell_size, path_num, max_path_load);
          }
      ...
      if (max_path_load != 0) {
        if (old_tags != nullptr) {
          dispatch_with_tag_indices_2d<dtype, true, true><<<grid_size, block_size, 0, stream>>>(
              src_data_ptr, dst_data_ptr, route_indices, loads, old_tags, new_tags, cell_num,
              cell_size, path_num, max_path_load);
        } else {
          dispatch_with_tag_indices_2d<dtype, true, false><<<grid_size, block_size, 0, stream>>>(
              src_data_ptr, dst_data_ptr, route_indices, loads, old_tags, new_tags, cell_num,
              cell_size, path_num, max_path_load);
        }
      } else {
        if (old_tags != nullptr) {
          dispatch_with_tag_indices_2d<dtype, false, true><<<grid_size, block_size, 0, stream>>>(
              src_data_ptr, dst_data_ptr, route_indices, loads, old_tags, new_tags, cell_num,
              cell_size, path_num, max_path_load);
        } else {
          dispatch_with_tag_indices_2d<dtype, false, false><<<grid_size, block_size, 0, stream>>>(
              src_data_ptr, dst_data_ptr, route_indices, loads, old_tags, new_tags, cell_num,
              cell_size, path_num, max_path_load);
        }
      }
