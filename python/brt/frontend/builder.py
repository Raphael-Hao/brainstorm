...
class GraphBuilder:
    ...
    def _add_edge_handle_source_node(
        self, _input, graph_inputs, ir_graph, output_remap, node_index
    ):
        if _input in output_remap:
            assert output_remap[_input].kind() == "aten::append"
            predecessor_node = output_remap[_input]
            assert predecessor_node in node_index, "predecessor node: {}".format(
                predecessor_node
            )
            src_node_idx = None
    ...
    def _add_edge(
        ...
        """
        is_single_input = (
            len([_input for _input in node.inputs()]) - (1 if ignore_first else 0)
        ) == 1
        ...
        for _input in node.inputs():
            if ignore_first:
                ...
                continue
            # handle source node
            src_node, src_node_idx = self._add_edge_handle_source_node(
                _input, graph_inputs, ir_graph, output_remap, node_index
            )
            ...
            # create edge
            ir_graph.add_edge(
                head=(src_node, src_node_idx), tail=(dst_node, dst_node_idx)
            )
    ...
    def create_prim_constant_node(self, ir_graph, node, module_name):
        ...
        if node.outputsAt(0).type().str() == "None":
            ...
            attrs = {
                "type": node.outputsAt(0).type().str(),
                "value": node.outputsAt(0).toIValue(),
            }
        self.global_seq += 1
        new_node = ir_graph.add_node(
            build_full_name(module_name, OpTypeName.Constant, self.global_seq),
            node.kind(),
            attrs,
        )
        return new_node
    ...
    def handle_prim_attr_node(self, node, module):
        ...
        attrs = {
            "name": node.s("name"),
            "input": node.inputsAt(0).debugName(),
            "value": value,
        }
    ...
    def handle_graph_nodes(
        ...
        def handle_if_condition(cond_tensor):
            ...
            def _generate_expr(tensor):
                if tensor.node().kind() == "prim::GetAttr":
                    ...
                    raise RuntimeError(
                        f"Unsupported op type {tensor.node().kind()} in if condition, "
        ...
        def handle_if_node(node):
            ...
            self.global_seq += 1
            new_node = ir_graph.add_node(
                build_full_name(module_name, "noop_identity", self.global_seq),
                "noop_identity",
            )
            self._add_edge(
                ir_graph,
                blocks[chosen_block].returnNode(),
                graph_inputs,
                node_index,
                new_node,
                output_remap,
        ...
        def handle_function_callmethod(node):
            ...
            if node.s("name") in ["forward", "forward__0"]:
                ...
                if submodule.inputsAt(0).debugName() == "self":
                    ...
                    if submodule.inputsAt(0).type().name() == "ModuleList":
                        ...
                        submodule_full_name = build_full_name(
                            module_name, list(reversed(module_name_space))
                        )
                        submodule_python_name = build_python_name(
                            module_python_name, list(reversed(module_name_space))
                        )
                        ...
                        raise RuntimeError(
                            "Unsupported module case: {}".format(
                                submodule.inputsAt(0).type().str()
                            )
                        )
                ...
                if submodule_full_name in shared_module_index:
                    ...
                    self.global_seq += 1
                    shared_node_name = build_full_name(
                        submodule_full_name, "", self.global_seq
                    )
                    shared_node_python_name = build_python_name(
                        submodule_python_name, self.global_seq
                    )
                    shared_type_operation = Operation.new(
                        "shared", {"reference": submodule_full_name}
                    )
                    ...
                    if subgraph is None:
                        ...
                        new_cell = Cell(
                            cell_name=submodule_full_name, parameters=sub_m_attrs
                        )
                ...
                script_method = getattr(
                    script_module, node.s("name")
                )  # <class 'torch._C.ScriptMethod'>

                # step #1: generate graph ir for this method
                method_ir_graph = Graph(
                    model=ir_model, graph_id=-100, name="temp_graph", _internal=True
                )
        ...
        def handle_single_node(node):
            ...
            if node.kind() == "prim::CallMethod":
                ...
                self.global_seq += 1
                func_node = ir_graph.add_node(
                    build_full_name(module_name, func_name, self.global_seq),
                ...
                node_index[node] = new_node
                self._add_edge(
                    ir_graph, node, graph_inputs, node_index, new_node, output_remap
                )
            elif node.kind() == "prim::GetAttr":
                node_type, attrs = self.handle_prim_attr_node(node, module)
                self.global_seq += 1
                new_node = ir_graph.add_node(
                    build_full_name(module_name, OpTypeName.Attr, self.global_seq),
                    node_type,
                    attrs,
                )
                node_index[node] = new_node
                ...
                aten_node = ir_graph.add_node(
                    build_full_name(module_name, aten_op_name, self.global_seq),
                    node.kind(),
                )
                node_index[node] = aten_node
                self._add_edge(
                    ir_graph, node, graph_inputs, node_index, aten_node, output_remap
                )
                ...
                aten_python_name = build_python_name(
                    module_python_name, aten_op_python_name
                )
        ...
        if node_index != {}:
            for _output in sm_graph.outputs():
                ...
                ir_graph.add_edge(
                    head=(node_index[_output.node()], src_node_idx),
            ...
            ir_graph.add_edge(
                head=(ir_graph.input_node, 0), tail=(ir_graph.output_node, None)
            )
    ...
    def merge_aten_slices(self, ir_graph):
        ...
        for head_node in head_slice_nodes:
            slot = 0
            new_slice_node = ir_graph.add_node(
                build_full_name(head_node.name, "merged"), OpTypeName.MergedSlice
            )
            ...
            while (
                ...
                and node.successors[0].operation.type == "aten::slice"
            ):
