...
def convert_to_visualize(graph_ir, vgraph):
    for name, graph in graph_ir.items():
        ...
        with vgraph.subgraph(name="cluster" + name) as subgraph:
            ...
            cell_node = {}
            ioput = {
                "_inputs": "{}-{}".format(name, "_".join(graph["inputs"])),
            ...
            for node_name, node_value in graph["nodes"].items():
                ...
                if value["type"] == "_cell":
                    cell_input_name = "{}-{}".format(
                        value["cell_name"],
                        "_".join(graph_ir[value["cell_name"]]["inputs"]),
                    )
                    cell_output_name = "{}-{}".format(
                        value["cell_name"],
                        "_".join(graph_ir[value["cell_name"]]["outputs"]),
                    )
