...
def is_layerchoice_node(ir_node: Node):
    if (
        ir_node is not None
        and isinstance(ir_node.operation, Cell)
        and ir_node.operation.parameters.get("mutation") == "layerchoice"
    ):
        return True
    else:
        return False
...
def _without_shape_info(node: Node):
    return (
        not node.operation.attributes["input_shape"]
        and not node.operation.attributes["output_shape"]
    )
            ...
            if node_graph is not None:
                ...
                for node_graph_node in node_graph.hidden_nodes:
                    new_node = Node(
                        graph,
                        node_graph_node.id,
                        node_graph_node.name,
                        node_graph_node.operation,
                        _internal=True,
                    )
                    ...
                    for input_node_edge in node_graph.input_node.outgoing_edges:
                        if input_node_edge.head_slot == in_edge.tail_slot:
                            graph.add_edge(
                                ...
                                tail=(
                                    id_to_new_node[input_node_edge.tail.id],
                                    input_node_edge.tail_slot,
                                ),
                            )
                ...
                for out_edge in node.outgoing_edges:
                    ...
                    for output_node_edge in node_graph.output_node.incoming_edges:
                        if output_node_edge.head_slot == out_edge.tail_slot:
                            graph.add_edge(
                                head=(
                                    id_to_new_node[output_node_edge.head.id],
                                    output_node_edge.head_slot,
                                ),
                ...
                for edge in node_graph.edges:
                    if (
                        edge.head == node_graph.input_node
                        or edge.tail == node_graph.output_node
                    ):
                        continue
                    new_head = id_to_new_node[edge.head.id]
                    new_tail = id_to_new_node[edge.tail.id]
                    Edge(
                        (new_head, edge.head_slot),
                        (new_tail, edge.tail_slot),
                        _internal=True,
                    )._register()
