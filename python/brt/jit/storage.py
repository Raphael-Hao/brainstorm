...
"""
    ...
    def add_kernel(self, kernel_in_json, overwrite=False):
        if overwrite:
            del_kernel_cmd = self.DEL_KERNEL_CMD.format(kernel_in_json["Key"])
            self.cursor.execute(del_kernel_cmd)
        add_kernel_cmd = self.ADD_KERNEL_CMD.format(
            kernel_in_json["Key"],
        ...
        self.cursor.execute(add_kernel_cmd)
    
    def init_kernel_cache_db(self):
        self.cursor.execute(self.INIT_DB_CMD)
    
    def query_kernel(self, kernel_key):
        query_kernel_cmd = self.QUERY_KERNEL_CMD.format(kernel_key, "GPU")
        self.cursor.execute(query_kernel_cmd)
