...
class FunctionFactory:
    @staticmethod
    def make_function(modules, sample_inputs=None, mode="eval", opt_level="none"):
        if mode == "eval":
            assert (
                sample_inputs is not None
            ), "sample_inputs must be provided in eval mode"
            if opt_level == "none":
                assert (
                    len(modules) == 1
                ...
                for subclass in ModuleInfo.__subclasses__():
                    ...
                    break
                jit_kernel = KernelFactory.make_kernel(
                    modules, sample_inputs, "forward", opt_level
                )
