...
class PrimConstant(PyTorchOperation):
    ...
    def to_forward_code(
        ...
        if self.parameters["type"] in ["None", "NoneType"]:
            ...
            raise RuntimeError(
                f'unsupported type of prim::Constant: {self.parameters["type"]}'
            )
...
class AtenTensors(PyTorchOperation):
    ...
    def to_forward_code(
        ...
        for idx, arg in enumerate(schemas[matched].arguments):
            if arg.name == "dtype":
                arg_str = (
                    f"dtype={scalar_type_to_pytorch_type[inputs_value[idx]]}"
                ...
                arg_str = (
                    f"device=torch.device({inputs[idx]})"
                    if inputs_value[idx] is not None
                    else ""
                )
            elif arg.name == "memory_format":
                arg_str = (
                    f"memory_format={mem_format[inputs_value[idx]]}"
                    if inputs_value[idx] is not None
                    else ""
                )
...
def _emit_args(args):
    ...
    return [
        (arg.name, str(arg.type), str(arg.default_value)) for arg in args
    ]  # if arg.name != 'out'
            ...
            if builtin is not None:
                ...
                for schema in schemas:
                    ...
                    if not _hidden(elem):
                        op_name = "aten::" + elem
                        if (
                            len(schema.arguments) > 0
                            and schema.arguments[0].name == "self"
                        ):
                            continue
    ...
    def _get_matched_args(_type, inputs):
        ...
        if len(matched) == 1:
            ...
            if has_same_arg_name(matched):
                ...
                raise RuntimeError(
                    f"tensor op type {_type} has more than one matched: {matched}"
                )
        else:
...
class TorchOps(PyTorchOperation):
    ...
    _op_args["aten::pixel_shuffle"] = [
        [("input", "Tensor", "None"), ("upscale_factor", "Optional[int]", "None")]
    ]
    ...
    def _get_matched_args(_type, inputs):
        ...
        if len(matched) == 1:
            ...
            if has_same_arg_name(matched):
                ...
                raise RuntimeError(
                    f"torch op type {_type} has more than one matched: {matched}"
                )
        else:
    ...
    def to_forward_code(
        ...
        args_str = ", ".join(
            [
                f"{name}={inputs[i]}" if t.startswith("Optional[") else f"{inputs[i]}"
                for i, (name, t, default) in enumerate(matched_args)
            ]
        )
...
class ToDevice(PyTorchOperation):
    ...
    def to_forward_code(
        self, field: str, output: str, inputs: List[str], inputs_value: List[Any]
    ) -> str:
        if self.overridden_device_repr is None:
