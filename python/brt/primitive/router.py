...
def trace_router(
    ...
    def wrap(cls_or_func):
        ...
        if isinstance(cls_or_func, type):
            cls_or_func = _trace_router_cls(
                cls_or_func, kw_only, inheritable=inheritable
            )
        elif _is_function(cls_or_func):
            # cls_or_func = _trace_router_func(cls_or_func, kw_only)
            raise NotImplementedError(
                "Tracing function is not supported for router yet."
            )
        else:
            raise TypeError(
...
def _trace_router_cls(base, kw_only, call_super=True, inheritable=False):
...
def _trace_router_func(func, kw_only):
