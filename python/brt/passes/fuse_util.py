    ...
    def __init__(
        ...
        if isinstance(model, nn.Conv2d) and model.bias is not None:
            ...
            if isinstance(model[0], nn.Conv2d) and model[0].bias is not None:
                if len(model) == 1:
                    ...
                    len(model) == 3
                    and isinstance(model[1], nn.BatchNorm2d)
                    and isinstance(model[2], nn.ReLU)
                ):
        ...
        if self.module_name == "Conv2dBias":
            ...
            w_conv = (
                self.get_parameter(f"0_weight")
                .clone()
                .view(model[0].out_channels, -1)
                .cuda()
            )
            w_bn = torch.diag(
                self.get_parameter(f"1_weight").div(
                    torch.sqrt(model[1].eps + model[1].running_var)
                )
            ).cuda()
            fusedweight = torch.mm(w_bn, w_conv).view(model[0].weight.size()).cuda()
            if (
                model[0].bias is not None
                and self.module_name == "Conv2dBiasBatchNormReLU"
            ):
                b_conv = model[0].bias
                self.inputs_templete["forward"].extend(
                    [
                ...
                b_bn = (
                    model[1].bias
                    - model[1]
                    .weight.mul(model[1].running_mean)
                    .div(torch.sqrt(model[1].running_var + model[1].eps))
                    .cuda()
                )
...
class FusedLayer(nn.Module):
    ...
    def __init__(
        ...
        self.input_indices = []
        self.output_indices = []
        for i, (module_name, output_shape, iter_module) in enumerate(
            zip(self.module_names, self.output_shapes, models)
        ):
            ...
            if module_name == "Conv2dBias":
                ...
                w_conv = (
                    self.get_parameter(f"m{i}_0_weight")
                    .clone()
                    .view(iter_module[0].out_channels, -1)
                    .cuda()
                )
                w_bn = torch.diag(
                    self.get_parameter(f"m{i}_1_weight").div(
                        torch.sqrt(iter_module[1].eps + iter_module[1].running_var)
                    )
                ).cuda()
                fusedweight = (
                    torch.mm(w_bn, w_conv).view(iter_module[0].weight.size()).cuda()
                )
                if (
                    iter_module[0].bias is not None
                    and module_name == "Conv2dBiasBatchNormReLU"
                ):
                    ...
                    iter_module[0].bias is None and module_name == "Conv2dBatchNormReLU"
                ):
                    ...
                    b_bn = (
                        iter_module[1].bias
                        - iter_module[1]
                        .weight.mul(iter_module[1].running_mean)
                        .div(
                            torch.sqrt(iter_module[1].running_var + iter_module[1].eps)
                        )
                        .cuda()
                    )
                    fusedbias = (b_conv + b_bn).cuda()
    ...
    def forward(self, inputs: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        for i in range(self.num_submodels):
            if inputs[i].size(0) == 0:
                self.inputs_templete["forward"][self.input_indices[i]] = torch.empty(
                    self.input_shapes[i], device="cuda"
                )
            else:
