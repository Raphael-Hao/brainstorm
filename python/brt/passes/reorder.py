    ...
    def run_on_graph(self):
        ...
        for node in copy_order_node:
            if self.is_scatter_node(node):
                ...
                set_data_dependency, edge_node, node_visited_map = bfs(
                    node.args[0][-1], node
                )
                edge_users = []
                edge_nodes_in_seq = []
                    ...
                    for new_key in key.users.copy().keys():
                        ...
                        for new_key1 in new_key.users.copy().keys():
                            if edge_node_in_seq in node.args[0]:
                                index_edge_node: int = node.args[0].index(
                                    edge_node_in_seq
                                )
                ...
                for tmp_node in node.args[0]:
                    if (
                        self.is_function_node(tmp_node)
                        and tmp_node.target == torch.zeros_like
                    ):
