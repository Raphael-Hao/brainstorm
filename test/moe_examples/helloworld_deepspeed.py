...
args.local_rank = (
    args.local_rank if args.local_rank >= 0 else int(os.environ.get("LOCAL_RANK", 0))
)
...
torch.manual_seed(0)
x = torch.tensor(
    torch.randn([batch_size, num_tokens, model_dim], dtype=torch.float32, device="cpu")
    .detach()
    .numpy(),
...
params_for_all_reduce = [
    ...
    if not hasattr(p, "skip_allreduce") and getattr(p, "requires_grad", False)
]
...
for i in range(num_steps):
    ...
    tflops = (
        (batch_size * num_tokens * model_dim * hidden_size)
        * 4
        * args.top
        * 3
        * 1e-12
    ...
    dist_print(
        "STEP-%s: loss = %.5f, step_time = %.6f sec, perf = %.2f tflops."
