...
extern "C" {
__global__ void __launch_bounds__(32) horizontal_matmul(
    float* __restrict__ placeholder_32, float* __restrict__ placeholder1_32,
    float* __restrict__ T_batch_matmul_NT_32,
    float* __restrict__ placeholder_16, float* __restrict__ placeholder1_16,
    float* __restrict__ T_batch_matmul_NT_16, float* __restrict__ placeholder_8,
    float* __restrict__ placeholder1_8, float* __restrict__ T_batch_matmul_NT_8,
    float* __restrict__ placeholder_4, float* __restrict__ placeholder1_4,
    float* __restrict__ T_batch_matmul_NT_4, float* __restrict__ placeholder_2,
    float* __restrict__ placeholder1_2, float* __restrict__ T_batch_matmul_NT_2,
    int n_32 = 0, int n_16 = 0, int n_8 = 0, int n_4 = 0, int n_2 = 0) {
  ...
  if (blockIdx.x < n_32 * 128) {
    matmul_32_1024_512(placeholder_32, placeholder1_32, T_batch_matmul_NT_32,
                       placeholder_d_shared, placeholder_shared, _blockIdx,
                       threadIdx);
  } else if (blockIdx.x < n_32 * 128 + n_16 * 32) {
    _blockIdx.x -= n_32 * 128;
    matmul_16_1024_512(placeholder_16, placeholder1_16, T_batch_matmul_NT_16,
                       placeholder_d_shared, placeholder_shared, _blockIdx,
                       threadIdx);
  } else if (blockIdx.x < n_32 * 128 + n_16 * 32 + n_8 * 64) {
    if (threadIdx.x < 16) {
      _blockIdx.x -= n_32 * 128 + n_16 * 32;
      matmul_8_1024_512(placeholder_8, placeholder1_8, T_batch_matmul_NT_8,
                        placeholder_d_shared, placeholder_shared, _blockIdx,
                        threadIdx);
    }
    ...
    if (threadIdx.x < 16) {
      ...
      matmul_4_1024_512(placeholder_4, placeholder1_4, T_batch_matmul_NT_4,
                        placeholder_d_shared, placeholder_shared, _blockIdx,
                        threadIdx);
    }
  } else if (blockIdx.x <
             n_32 * 128 + n_16 * 32 + n_8 * 64 + n_4 * 32 + n_2 * 16) {
    _blockIdx.x -= n_32 * 128 + n_16 * 32 + n_8 * 64 + n_4 * 32;
    matmul_2_1024_512(placeholder_2, placeholder1_2, T_batch_matmul_NT_2,
                      placeholder_d_shared, placeholder_shared, _blockIdx,
                      threadIdx);
  }
}
...
int main(int argc, char const* argv[]) {
  ...
  for (auto i = 0; i < 1024; ++i) {
    horizontal_matmul<<<blocks_per_grid, threads_per_block, 0, stream>>>(
        A_d, B_d, C_d_grand, A_d, B_d, C_d_grand, A_d, B_d, C_d_grand, A_d, B_d,
        C_d_grand, A_d, B_d, C_d_grand, 1, 1, 1, 1, 1);
  }
